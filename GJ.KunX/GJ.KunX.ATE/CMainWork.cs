    ...
    public class CMainWork : IMainWork
    {
            ...
            try
            {
                ...
                for (int idNo = 0; idNo < CGlobalPara.C_STAT_MAX; idNo++)
                {
                    _statATE[idNo] = new CStat(idNo, "<"+  CLanguage.Lan("ATE工位") + (idNo + 1).ToString() + ">",
                                               CKunXApp.ATE_FlowId, CKunXApp.ATE_FlowName, uutMax);
                }
                       ...
                       if (_devATE.DoRun != ERUN.结束)
                        ...
                        for (int i = 0; i < _statATE[idNo].SerialNo.Count; i++)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    _statATE[idNo].Result[i] = CKunXApp.ATE_FlowId;
                                    uutPass = false;
                                }
                            }
                        }
                        ...
                        if (CGlobalPara.SysPara.Mes.Connect)
                        {
                            ...
                            {
                                //Task.Factory.StartNew(() => TranSnToMES(idNo));
        ...
        private bool GetRunTestStep()
        {
            try
            {
                ...
                int stepNum = CKunXApp.SlotMax / CGlobalPara.SysPara.Dev.DevMax / CGlobalPara.SysPara.Dev.ChanMax;
        ...
        private bool UpdateFixtureResult(int idNo)
        {
            try
            {
                ...
                fixture.Base.FlowName = CKunXApp.ATE_FlowName;
        ...
        /// <returns></returns>
        private bool TranSnToMES(int idNo)
        {
            try
            {
                string er = string.Empty;
                ...
                Log(CLanguage.Lan("治具ID") + "【" + _statATE[idNo].IdCard + "】"+ CLanguage.Lan("上传ATE结果结束:") +
