    ...
    public class CMainWork : IMainWork
    {
            ...
            {                
                ...
                for (int idNo = 0; idNo < CGlobalPara.C_HP_STAT_MAX; idNo++)
                {
                    _statHipot[idNo] = new CStat(idNo, "<"+ CLanguage.Lan("高压检查工位")  + (idNo + 1).ToString() + ">",
                                                       ...
                                                       CKunXApp.HIPOT_FlowId, CKunXApp.HIPOT_FlowName, CKunXApp.SlotMax);                                                    
                ...
                for (int idNo = 0; idNo < CGlobalPara.C_ATE_STAT_MAX; idNo++)
                {
                    _statATE[idNo] = new CStat(CGlobalPara.C_HP_STAT_MAX + idNo, "<"+ CLanguage.Lan("ATE检查工位") + (idNo + 1).ToString() + ">",
                                                                              ...
                                                                              CKunXApp.ATE_FlowId, CKunXApp.ATE_FlowName,CKunXApp.SlotMax);                                                                             
                ...
                _statFinal = new CStatHub(CGlobalPara.C_HP_STAT_MAX + CGlobalPara.C_ATE_STAT_MAX, 
                                             ...
                                             CKunXApp.UNLOAD_FlowId, CKunXApp.UNLOAD_FlowName, CKunXApp.SlotMax);                                      
                ...
                _serReponse = new CSerSocket(CKunXApp.SlotMax);
        ...
        private bool OpenGJWebServer()
        {
            try
            {
                ...
                if (!CWeb2.CheckSystem(CKunXApp.UlrWeb, out version, out er))
                {
                    Log(CLanguage.Lan("连接冠佳WEB") + "【" + CKunXApp.UlrWeb + "】"+ CLanguage.Lan("错误:") + er, udcRunLog.ELog.NG);
                    return false;
                }

                Log(CLanguage.Lan("连接冠佳WEB") + "【" + CKunXApp.UlrWeb + "】"+ CLanguage.Lan("正常"), udcRunLog.ELog.Action);
        ...
        private bool CheckHipotTestReady()
        {
            try
            {
                ...
                if (CGlobalPara.C_HP_STAT_MAX>1 && readyNum == CGlobalPara.C_HP_STAT_MAX)
                {
                    ...
                    OnUIMainArgs.OnEvented(new CUIUserArgs<CUIMainArgs>(idNo, CKunXApp.HIPOT_FlowName, UIMainArgs));
        ...
        private bool CheckATETestReady()
        {
            try
            {
                ...
                if (CGlobalPara.C_ATE_STAT_MAX >1 && readyNum == CGlobalPara.C_ATE_STAT_MAX)
                {
                    ...
                    OnUIMainArgs.OnEvented(new CUIUserArgs<CUIMainArgs>(idNo, CKunXApp.ATE_FlowName, UIMainArgs));
                ...
                if (handMode == EPLCMode.自动下机) //自动下机模式
                {
                    if (_threadPLC.rREG_Val(EPLCINP.自动下机位治具到位.ToString()) != 1)
                    {
                        ...
                        if (_threadPLC.rREG_Val(EPLCINP.自动下机位治具到位光电.ToString()) == 0)
                        {
                            ...
                            OnUIMainArgs.OnEvented(new CUIUserArgs<CUIMainArgs>(idNo, CKunXApp.UNLOAD_FlowName, UIMainArgs));
                        }                           
                        return true;
                    }
                }
        ...
        private void OnHubFixtureReady(int idNo, CStatHub hub)
        {
            try
            {
                ...
                //提示是否变更机种?
                if (CGlobalPara.SysPara.Para.ChkModel && hub.FlowName == CKunXApp.HIPOT_FlowName)
                {
        ...
        private void OnStatTestStart(int idNo, CStatTest hub)
        {
            try
            {
                ...
                if (hub.FlowName == CKunXApp.HIPOT_FlowName) //高压位启动测试                
                {
                    CSerSocket.CSAT serTCP = new CSerSocket.CSAT(CKunXApp.SlotMax);
                    serTCP.StatName = CGlobalPara.SysPara.Dev.HP_TCP;
                    serTCP.SubNo = idNo + 1;
                    serTCP.ModelName = hub.OrderName;
                ...
                else if (hub.FlowName == CKunXApp.ATE_FlowName) //ATE启动测试
                {
                    if (CGlobalPara.SysPara.Dev.AteDevMax == 0)
                    {
                        CSerSocket.CSAT serTCP = new CSerSocket.CSAT(CKunXApp.SlotMax);
                    ...
                    else
                    {
                        ...
                        {
                            int slotMax = CKunXApp.SlotMax / 2;
        ...
        private void OnStatTestEnd(int idNo, CStatTest hub)
        {
            try
            {
                ...
                if (hub.FlowName == CKunXApp.HIPOT_FlowName)
                {
                    _WarnRate[0].TTNum += ttNum;
                    _WarnRate[0].PassNum += passNum;
                }
                else if (hub.FlowName == CKunXApp.ATE_FlowName)
                {
        ...
        private void OnFinalFixtureReady(EPLCMode handMode)
        {
            try
            {
                ...
                for (int i = 0; i < fixture.Para.Count; i++)
                {
                    ...
                    {
                        if (_statFinal.Result[i] != 0 || _statFinal.ResultId[i] != CKunXApp.UNLOAD_FlowId - 1)
                        {
                            uutPass = false;
                        }
                    }
                        ...
                        if (_statFinal.SerialNo[i] != string.Empty)
                        {
                            ...
                            else
                            {
                                if (resultList[i] == 0)
                                {
                                    ...
                                    _statFinal.ResultId[i] = CKunXApp.UNLOAD_FlowId - 1;
            ...
            try
            {
                ...
                for (int i = 0; i < _statFinal.SerialNo.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (_statFinal.ResultId[plcNo] == CKunXApp.ATE_FlowId) //PASS
                            {
                                regVal[i] = 1;
                            }
                            else if (_statFinal.ResultId[plcNo] == CKunXApp.HIPOT_FlowId)  //ATE未测试
                            {
                                regVal[i] = 4;
                                failNum++;
                            }
                            else if (_statFinal.ResultId[plcNo] == CKunXApp.BI_FlowId)  //HIPOT未测试
                            {
                                regVal[i] = 3;
                                failNum++;
                            }
                            else if (_statFinal.ResultId[plcNo] == CKunXApp.PRETEST_FlowId)  //BI未测试
                            {
                                regVal[i] = 2;
                                failNum++;
                            }
                        ...
                        else if (_statFinal.ResultId[plcNo] == CKunXApp.PRETEST_FlowId) //PT FAIL
                        {
                            regVal[i] = 5;
                            failNum++;
                        }
                        else if (_statFinal.ResultId[plcNo] == CKunXApp.BI_FlowId)  //BI FAIL
                        {
                            regVal[i] = 2;
                            failNum++;
                        }
                        else if (_statFinal.ResultId[plcNo] == CKunXApp.HIPOT_FlowId)  //HP FAIL
                        {
                            regVal[i] = 3;
                            failNum++;
                        }
                        else if (_statFinal.ResultId[plcNo] == CKunXApp.ATE_FlowId)  //ATE FAIL
                        {
                            regVal[i] = 4;
                            failNum++;
                        }
        ...
        private void LoadYield()
        {
            ...
            _statYield.Add(CKunXApp.PRETEST_FlowName, new CYield());

            _statYield.Add(CKunXApp.BI_FlowName, new CYield());

            _statYield.Add(CKunXApp.HIPOT_FlowName, new CYield());

            _statYield.Add(CKunXApp.ATE_FlowName, new CYield());
        ...
        private void CalStationFlowYield(out List<int> ttNum, out List<int> failNum)
        {
            ...
            try
            {
                ...
                yieldList.Add(CKunXApp.ATE_FlowName, 3);

                yieldList.Add(CKunXApp.HIPOT_FlowName, 2);

                yieldList.Add(CKunXApp.BI_FlowName, 1);

                yieldList.Add(CKunXApp.PRETEST_FlowName, 0);

                Dictionary<string, int> flowList = new Dictionary<string, int>();

                flowList.Add(CKunXApp.ATE_FlowName, CKunXApp.ATE_FlowId);

                flowList.Add(CKunXApp.HIPOT_FlowName, CKunXApp.HIPOT_FlowId);

                flowList.Add(CKunXApp.BI_FlowName, CKunXApp.BI_FlowId);

                flowList.Add(CKunXApp.PRETEST_FlowName, CKunXApp.PRETEST_FlowId);
        ...
        private bool CheckTestConnect()
        {
            try
            {
                ...
                CSFCS mes = new CSFCS(0, "WebAPI", CKunXApp.Custom);
                ...
                CKunXApp.OrderName = er;

                Log(CLanguage.Lan("连接客户MES服务器正常") + "[" + CKunXApp.OrderName + "]", udcRunLog.ELog.Action);
        ...
        private bool CheckPLCAlarm()
        {
            try
            {
                ...
                Alarm.Base.LineNo = CKunXApp.LineNo;
                Alarm.Base.LineName = CKunXApp.LineName;
        ...
        private void InitialIot()
        {
            try
            {
                ...
                InitialIotDevice(CKunXApp.IoT_Server, CKunXApp.Iot_Factory, devList);
