...
namespace GJ.KunX.LOADUP
{
    public class CMainWork:IMainWork
    {
        ...
        public override bool InitialRunPara()
        {
            ...
            CKunXApp.OrderName = string.Empty;
        ...
        private bool load_user_info(out string er)
        {
            ...
            try
            {
                ...
                _statInfo = new CStatInfo(CKunXApp.SlotMax);

                _statManualScan = new CStatHub(0, "<" + CLanguage.Lan("人工扫描绑定位") + ">",
                                                  CKunXApp.LOADUP_FlowId, CKunXApp.LOADUP_FlowName, CKunXApp.SlotMax);
                _statManualScan.Base.IdCardAddr = 1;
                _statManualScan.Base.SnBarNo[0] = new int[CKunXApp.SlotMax / 2];
                _statManualScan.Base.SnBarNo[1] = new int[CKunXApp.SlotMax / 2];
                string[] Str1 = CKunXApp.LOADUP_Manual_SnNo1.Split(',');
                string[] Str2 = CKunXApp.LOADUP_Manual_SnNo2.Split(',');
                for (int i = 0; i < CKunXApp.SlotMax / 2; i++)
                {
                ...
                _statAutoScan.Base.SnBarNo[0] = new int[CKunXApp.SlotMax / 2];
                _statAutoScan.Base.SnBarNo[1] = new int[CKunXApp.SlotMax / 2];
                string[] Str3 = CKunXApp.LOADUP_Auto_SnNo1.Split(',');
                string[] Str4 = CKunXApp.LOADUP_Auto_SnNo2.Split(',');
                for (int i = 0; i < CKunXApp.SlotMax / 2; i++)
                {
                ...
                _statPreTest = new CStat(1, "<"+  CLanguage.Lan("初测测试检查位") + ">", "<" + CLanguage.Lan("初测通电测试位") + ">", 
                                                  CKunXApp.PRETEST_FlowId, CKunXApp.PRETEST_FlowName, CKunXApp.SlotMax);
                _statPreTest.hub.Base.IdCardAddr = 2;

                _statInBI = new CStatHub(2, "<"+ CLanguage.Lan("进老化检查位") + ">", CKunXApp.BI_FlowId, CKunXApp.BI_FlowName, CKunXApp.SlotMax);
        ...
        private bool OpenELoadMon()
        {
            try
            {
                ...
                if (_devELoad == null)
                {
                    ...
                    int AddrMax = CKunXApp.SlotMax / _devELoad.maxCH;
        ...
        private bool OpenGJWebServer()
        {
            try
            {
                ...
                {
                    Log(CLanguage.Lan("连接冠佳Web") + "【" + CKunXApp.UlrWeb + "】" + 
                                                             CLanguage.Lan("错误:") + er, udcRunLog.ELog.NG);
                    return false;
                }
        ...
        private void OnSnComRecved(object sender, CRecvArgs e)
        {
            try
            {
                ...
                int maxScan = CKunXApp.SlotMax / 2;
        ...
        private void OnScanTask()
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (_statAutoScan.Para.CurSnNo[0] < CKunXApp.SlotMax / 2)
                    {
                        _threadPLC.addREGWrite(EPLCOUT.发送自动扫条码位置号.ToString(), _statAutoScan.Para.CurSnNo[0] + 1);
                        continue;
                    }
            ...
            try
            {
                for (int slot = 0; slot < _statPreTest.test.Fixture.SerialNo.Count; slot++)
                {
                    ...
                    if (_statPreTest.test.Fixture.SerialNo[slot] != string.Empty)
                    {
                        ...
                        {
                            _statPreTest.test.Fixture.Result[slot] = CKunXApp.PRETEST_FlowId;
                        }
        ...
        private void OnInBIStatReady()
        {
            try
            {
                ...
                for (int i = 0; i < _statInBI.Fixture.SerialNo.Count; i++)
                {
                    ...
                    {
                        if (_statInBI.Fixture.Result[i] == 0 && _statInBI.Fixture.ResultId[i] == CKunXApp.PRETEST_FlowId)
                        {
                            HaveUUT = true;
                        }
                        else
                        {
                            _statInBI.Fixture.Result[i] = CKunXApp.PRETEST_FlowId;
        ...
        private bool local_db_SaveFixtureInfo(CStatHub statHub, out string er)
        {
            ...
            try
            {
                ...
                string statName = CKunXApp.PRETEST_FlowName;
        ...
        private bool BandSnToFixture(CWeb2.EFixtureType fixType, CStatHub statHub, out string er)
        {
            ...
            try
            {
                ...
                fixture.Base.FlowName = CKunXApp.LOADUP_FlowName;
                fixture.Base.FlowGuid = CNet.HostName();
                fixture.Base.LineNo = CKunXApp.LineNo;
                fixture.Base.LineName = CKunXApp.LineName;
                fixture.Base.Model = _runModel.Base.Model;
                fixture.Base.OrderName = CKunXApp.OrderName;
                fixture.Base.MesFlag = (CGlobalPara.SysPara.Mes.Connect ? 1 : 0);
                fixture.Base.SnType = CWeb2.ESnType.外部条码;
                fixture.Base.MaxSlot = CKunXApp.SlotMax;
        ...
        private bool UpdateFixtureResult(out string er)
        {
            try
            {
                ...
                CWeb2.CFixture fixture = new CWeb2.CFixture();
                fixture.Base.FlowIndex = 0;
                fixture.Base.FlowName = CKunXApp.PRETEST_FlowName;
                fixture.Base.FlowGuid = CNet.HostName();
                fixture.Base.SnType = CWeb2.ESnType.外部条码;
        ...
        private bool CheckSnFormWeb(string serialNo, out string er)
        {
            try
            {
                ...
                if (!CWeb2.CheckSn(CKunXApp.LOADUP_FlowName, serialNo, out er))
                    return false;
        ...
        private bool CheckSn(string Sn,out string er)
        {
            ...
            try
            {
                ...
                CSFCS.CSnInfo Info = new CSFCS.CSnInfo()
                {
                    ...
                    StatName = CKunXApp.PRETEST_FlowName,
                    SerialNo = Sn,
                ...
                {
                    Info.StatName = CKunXApp.BI_FlowName;
                ...
                {
                    Info.StatName = CKunXApp.HIPOT_FlowName;
                ...
                {
                    Info.StatName = CKunXApp.ATE_FlowName;
        ...
        private bool CheckSafeDog(out string er)
        {
            ...
            try
            {
                ...
                if (leftDay <= 0)
                {
                    er = CLanguage.Lan("软件使用期限到期,请与我司业务人员联系") + CKunXApp.DogLiaisons;
                    return false;

                }
                else if (leftDay < CKunXApp.DogDayLimit)
                {
                    er =  CLanguage.Lan("软件使用期限还剩") + leftDay.ToString() + CLanguage.Lan("天") + ","+ 
                          CLanguage.Lan("请及时与我司业务人员联系") + CKunXApp.DogLiaisons;
                    return false;
                }
            ...
            try
            {
                ...
                else if (leftDay < CKunXApp.DogDayLimit)
                {
                    MessageBox.Show(CLanguage.Lan("软件使用期限还剩") + leftDay.ToString() + CLanguage.Lan("天") + ","+ 
                                    CLanguage.Lan("请及时与我司业务人员联系") + CKunXApp.DogLiaisons, "Tip",
                                     MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
