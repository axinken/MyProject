    ...
    public class CMainWork : IMainWork
    {
        ...
        private bool load_user_info(out string er)
        {
            ...
            try
            {
                ...
                _statHP = new CStat(_idNo, "<" + CLanguage.Lan("高压测试工位") + ">",
                                                CKunXApp.HIPOT_FlowId,
                                                CKunXApp.HIPOT_FlowName, 
                                                CKunXApp.SlotMax);
                ...
                for (int idNo = 0; idNo < CGlobalPara.SysPara.Dev.HPDevMax; idNo++)
                {
                    int slotNum = CKunXApp.SlotMax / CGlobalPara.SysPara.Dev.HPDevMax;
        ...
        private bool OpenGJWebServer()
        {
            try
            {
                ...
                Log(CLanguage.Lan("连接冠佳WEB") + "【" + CKunXApp.UlrWeb + "】OK", udcRunLog.ELog.Action);
        ...
        private bool CheckStatTest()
        {
            try
            {
                ...
                int slotMax = CKunXApp.SlotMax / CGlobalPara.SysPara.Dev.HPDevMax;
                    ...
                    case ERUN.测试:
                        ...
                        for (int i = 0; i < _statHP.SerialNo.Count; i++)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    _statHP.Result[i] = CKunXApp.HIPOT_FlowId;
                                    uutPass = false;
                                }
                                //记录测试数据
        ...
        private bool GetRunTestStep(int idNo)
        {
            try
            {
                ...
                int stepNum = CKunXApp.SlotMax / CGlobalPara.SysPara.Dev.HPDevMax / CGlobalPara.SysPara.Dev.HPChanMax;
        ...
        {
            serReponse = new CSerReponse(CKunXApp.SlotMax);
            ...
            try
            {
                ...
                if (CGlobalPara.SysPara.Dev.TcpMode == 0)
                {
                    ...
                    socketReponse.UUT_NUM = CKunXApp.SlotMax;
                    socketReponse.UUT = new SOCKET_UUT[CKunXApp.SlotMax];
        ...
        private bool UpdateFixtureResult()
        {
            try
            {
                ...
                fixture.Base.FlowName = CKunXApp.HIPOT_FlowName;
            ...
            try
            {
                ...
                for (int slotNo = 0; slotNo < _statHP.SerialNo.Count; slotNo++)
                {
                    ...
                    CSFCS.CSnData SnData = new CSFCS.CSnData()
                    {
                        ...
                        StatName = CKunXApp.HIPOT_FlowName,
                        ...
                        OrderName = _statHP.ModelName,
                        Fixture = _statHP.IdCard + "_" + (slotNo+1).ToString("D2"),
                    ...
                    for (int i = 0; i < _statHP.HpResult[slotNo].mVal.Count; i++)
                    {
                        ...
                        if (_runModel.Para.Step[i].name == EStepName.IR)
                        {
                            if (unit == "M0hm")
                            {
                                value = _statHP.HpResult[slotNo].mVal[i].value.ToString("0.0");
                            }
                            else
                            {
                                if (_statHP.HpResult[slotNo].mVal[i].value > 1000)
                                    ...
                                    value = (_statHP.HpResult[slotNo].mVal[i].value * 1000).ToString();
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            SFCS.FrmAction.AlarmFlag = 1;

                            SFCS.FrmAction.AlarmInfo = "MES系统网络异常,请检查网络是否正常?";
                ...
                if (!uutPass && SFCS.FrmAction.AlarmFlag != 1)
                {
                    SFCS.FrmAction.AlarmFlag = 2;

                    SFCS.FrmAction.AlarmInfo = "治具ID[" + _statHP.IdCard + "]上传MES结果所有FAIL,请检查.";
                }
