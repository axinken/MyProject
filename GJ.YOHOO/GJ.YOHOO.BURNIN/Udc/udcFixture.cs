                ...
                else
                {
                    if (_runUUT.Led[i].failResult == 0)
                    {
                        if(_runUUT.Para.DoRun == EDoRun.正在老化)
                           labUUT[i].BackColor = Color.Green;
                        else
                           labUUT[i].BackColor = Color.Lime;
                    }
                    else
                    {
                        if (_runUUT.Led[i].failResult == 2)  //当机
                        {
                            labUUT[i].BackColor = Color.SandyBrown;
                        }
                        else
                        {
                            if (_runUUT.Led[i].unitV < 2)  //无输出
                            {
                                labUUT[i].BackColor = Color.Teal;
                            }
                            else if(_runUUT.Led[i].unitV< _runUUT.Led[i].vMin)  //欠压
                            {
                                labUUT[i].BackColor = Color.Red;
                            }
                            else if (_runUUT.Led[i].unitV > _runUUT.Led[i].vMin)  //过压
                            {
                                labUUT[i].BackColor = Color.RoyalBlue;
                            }
                            else if (_runUUT.Led[i].unitA < _runUUT.Led[i].Imin)  //欠流
                            {
                                labUUT[i].BackColor = Color.Yellow;
                            }
                            else if (_runUUT.Led[i].unitA > _runUUT.Led[i].Imax)  //过流
                            {
